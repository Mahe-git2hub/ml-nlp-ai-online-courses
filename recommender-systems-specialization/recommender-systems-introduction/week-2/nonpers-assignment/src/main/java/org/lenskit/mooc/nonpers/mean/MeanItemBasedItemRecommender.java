...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        TreeMap<Double,List<Long>> ratingToItem = new TreeMap<>();
        for(Long item: items) {
            Double rating = model.getMeanRating(item);
            List<Long> itemsList = ratingToItem.get(rating);
            if(itemsList==null) {
                itemsList = new ArrayList<>();
            }
            itemsList.add(item);
            ratingToItem.put(rating,itemsList);
        }
        for(int i=1;i<=n;) {
            Map.Entry<Double,List<Long>> ratingObj =  ratingToItem.lastEntry();
            Double rating = ratingObj.getKey();
            ratingToItem.remove(rating);
            List<Long> itemsList = ratingObj.getValue();

            int j=0;
            for(;i<=n && j < itemsList.size();) {
                results.add(new BasicResult(itemsList.get(j),rating));
                j++;
                i++;
            }
        }
