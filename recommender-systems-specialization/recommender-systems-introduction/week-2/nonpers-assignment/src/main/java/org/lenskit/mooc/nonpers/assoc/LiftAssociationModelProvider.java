    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                double pYnX = Sets.intersection(yUsers, xUsers).size() / (double)(allUsers.size());
                double px = xUsers.size()/(double)(allUsers.size());
                double py = yUsers.size()/(double)(allUsers.size());
                double assoc = pYnX / (px*py);
                itemScores.put(yId,assoc);
