...
public class AssociationItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, long refItem, LongSet candidates) {
        ...
        TreeMap<Double,List<Long>> assocToItem = new TreeMap<>();
        for(Long item: candidates) {
            Double itemAssociation = model.getItemAssociation(refItem,item);
            List<Long> itemsList = assocToItem.get(itemAssociation);
            if(itemsList==null) {
                itemsList = new ArrayList<>();
            }
            itemsList.add(item);
            assocToItem.put(itemAssociation,itemsList);
        }
        Map<Double,List<Long>> assocDesc = assocToItem.descendingMap();
        for(int i=1;i<=n;) {
            Map.Entry<Double,List<Long>> assocObj =  assocToItem.lastEntry();
            Double assoc = assocObj.getKey();
            assocToItem.remove(assoc);
            List<Long> itemsList = assocObj.getValue();

            int j=0;
            for(;i<=n && j < itemsList.size();) {
                results.add(new BasicResult(itemsList.get(j), assoc));
                j++;
                i++;
            }
        }
